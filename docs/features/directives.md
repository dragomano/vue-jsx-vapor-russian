...
> [!WARNING]
> Значение по умолчанию при деструктуризации области видимости слота применяется только в том случае, если свойство переименовано, например: `v-slot={({ foo: bar = '' })}`. Сокращённая форма `v-slot={({ foo = '' })}` не поддерживается, поскольку выражение области видимости разбирается как литерал объекта, а `{ foo = '' }` не является допустимым синтаксисом.
