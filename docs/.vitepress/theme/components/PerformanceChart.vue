...
const { title = 'Операций в секунду' } = defineProps<{
  title?: string
}>()
...
onMounted(async () => {
  const res = await fetch(`${import.meta.env.BASE_URL}bench-results.json`)
...
<template>
  <div style="height: 300px; max-width: 900px; margin: 0 auto; padding: 0 20px">
    ...
    <p
      ...
      "
    >
      * Бенчмарки проводились на M1 Pro с Node.js v23 (
      <a
